...
def get_scheduler(optimizer, opt, cam=None): #hmg改，更改cam的scheduler
...
class PatchSampleF(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        ...
        self.beishu=[32,32,16,8,8]#self.opt.minmaxq
    ...
    def forward(self, feats, num_patches=64, patch_ids=None, minmaxidxy=None):
        return_ids = []
        return_feats = []
        if self.use_mlp and not self.mlp_init:
            self.create_mlp(feats)
        for feat_id, feat in enumerate(feats): #262,256,128,64,64,1
            B, H, W = feat.shape[0], feat.shape[2], feat.shape[3]
            feat_reshape=None
            feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2) #n,h*w,c
            if num_patches > 0:
                ...
                if minmaxidxy is not None and patch_id.shape[0]>1:
                    patch_id = patch_id.unsqueeze(dim=0).repeat([B,1])
                    # patch_id_2[:,:16] = torch.round(minmaxidxy[:,0,:]*self.beishu[feat_id]*W+minmaxidxy[:,1,:]*self.beishu[feat_id])
                    # patch_id[:,:16] = torch.round((minmaxidxy[:,0,:]*W+minmaxidxy[:,1,:])*self.beishu[feat_id])
                    a = torch.tensor((minmaxidxy[:,0,:]*W+minmaxidxy[:,1,:]), dtype=torch.int)*self.beishu[feat_id]
                    # print(f'{a.max()}, {a.min()}')
                    # assert(a.max()<=patch_id.max())

                    # if feat.shape[2]==256:#用于画q
                    #     f=open('a.txt','w')
                    #     for i in range(len(a[0])):
                    #         f.write(str(int(a[0][i]))+',')
                    #         # print(int(a[0][i]))
                    #     for i in range(len(patch_id[0])):
                    #         f.write(str(int(patch_id[0][i]))+',')
                    #         # print(int(patch_id[0][i]))
                    #     f.close()
                        # print('运行train copy')

                    patch_id[:,:16]=a.detach()
                    # for i in range(B):
                    #     patch_id[i,:16] = torch.round((minmaxidxy[i,0,:]*W+minmaxidxy[i,1,:])*self.beishu[feat_id])
                if patch_id.shape[0] == B and len(patch_id.shape)>1:#len(patch_id.shape)>1  是因为当使用global的向量时会出错
                    x_sample = torch.ones(size=[B,patch_id.shape[1],feat_reshape.shape[2]],device=feat_reshape.device)
                    for i in range(B):
                        x_sample[i,:,:] = feat_reshape[i, patch_id[i], :]
                    x_sample = x_sample.flatten(0,1).to(feat_reshape.device)
                else:
...
from . import resnet_gai #hmg添加
def define_G_encdec(arch, input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
             init_gain=0.02, no_antialias=False, no_antialias_up=False, gpu_ids=[], opt=None):
    net = G_all(arch, input_nc, output_nc, ngf, netG, norm, use_dropout, init_type,
             init_gain, no_antialias, no_antialias_up, gpu_ids, opt)
    return net

def define_G_enc():
    net = None
    net = resnet_gai.ResNet()
    return net

def define_G_dec(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
             init_gain=0.02, no_antialias=False, no_antialias_up=False, gpu_ids=[], opt=None):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
    # net = resnet_gai.ResNet_decoder()
    net = ResnetGenerator_dec(input_nc, output_nc, ngf         , use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)
    return init_net(net, init_type, init_gain, gpu_ids, initialize_weights=('stylegan2' not in netG))

class G_all(nn.Module):
    def __init__(self,arch, input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
             init_gain=0.02, no_antialias=False, no_antialias_up=False, gpu_ids=[], opt=None):
        super(G_all,self).__init__()
        self.enc = define_G_enc(arch)
        if len(gpu_ids) > 0:
            assert(torch.cuda.is_available())
            self.enc.to(gpu_ids[0])
        self.adjust = init_net(Layer_adjust(2048, 2048), init_type, init_gain, gpu_ids, initialize_weights=('stylegan2' not in netG))
        self.dec = define_G_dec(input_nc, output_nc, ngf, netG, norm, use_dropout, init_type, init_gain, no_antialias, no_antialias_up, gpu_ids, opt)

    def forward(self, input, layers=[], encode_only=False):
        feats = []
        if len(layers) >0:
            x, feats = self.enc(input,layers)
            if encode_only:
                return feats
            if self.adjust != None:
                x = self.adjust(x)
...
class Layer_adjust(nn.Module):

    """使用《Deep Model Reassembly》里的说法用BN 1卷积 和 ReLU(gai) 来调整通道"""
    # def __init__(self, input_nc, output_nc,):
    #     super(Layer_adjust, self).__init__()
        
    #     model=[nn.Conv2d(input_nc,input_nc*2,1,1,0),
    #            nn.BatchNorm2d(input_nc*2),
    #            nn.Conv2d(input_nc*2,output_nc,1,1,0),
    #            nn.BatchNorm2d(output_nc),
    #            nn.ReLU()]
    #     self.model = nn.Sequential(*model)
    # def forward(self, input):
    #     return self.model(input)
    """使用resnet50里的Bottleneck块"""
    def __init__(self, input_nc, output_nc):
        super(Layer_adjust, self).__init__()
        self.conv1 = nn.Conv2d(input_nc, int(input_nc/4), kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(int(input_nc/4))
        self.conv2 = nn.Conv2d(int(input_nc/4), int(input_nc/4), kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(int(input_nc/4))
        self.conv3 = nn.Conv2d(int(input_nc/4), output_nc, kernel_size=1, bias=False)
